...
#
# Usage: deploy/install.sh <tessera-home> [owner/repo]
#   tessera-home  absolute (or relative, resolved to absolute) path to the
#                 install root — parent of releases/, current, data/, .env.
#                 e.g. ~/opt/tessera
#   owner/repo    defaults to TESSERA_UPDATE_REPO's own default
#                 (s3ntin3l8/tessera-session-manager) — override only for a
#                 fork publishing releases elsewhere.

set -euo pipefail

TESSERA_HOME_INPUT="${1:?usage: deploy/install.sh <tessera-home> [owner/repo]}"
REPO="${2:-s3ntin3l8/tessera-session-manager}"
...
echo "==> Installing into $TESSERA_HOME"

mkdir -p "$TESSERA_HOME/releases" "$TESSERA_HOME/data/sessions"
...
RELEASE_DIR="$TESSERA_HOME/releases/$VERSION"
...
mv -T "$TESSERA_HOME/current.tmp" "$TESSERA_HOME/current"

if [ -f "$TESSERA_HOME/.env" ]; then
  echo "==> $TESSERA_HOME/.env already exists, leaving it as-is"
else
  echo "==> Writing $TESSERA_HOME/.env"
...
DATABASE_URL=file:$TESSERA_HOME/data/app.db
SESSIONS_DIR=$TESSERA_HOME/data/sessions
TESSERA_HOME=$TESSERA_HOME
...
sed \
  -e "s#^WorkingDirectory=.*#WorkingDirectory=$TESSERA_HOME/current#" \
  -e "s#^ExecStart=.*#ExecStart=$NODE_PATH dist/server.js#" \
  -e "s#^EnvironmentFile=.*#EnvironmentFile=$TESSERA_HOME/.env#" \
